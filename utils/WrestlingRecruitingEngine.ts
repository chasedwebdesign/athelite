// =========================================================================
// 🤼 WRESTLING POSITIONAL RECRUITING ENGINE & PERFORMANCE BASES
// =========================================================================

export type WrestlingWeightClass = 
  '106 lbs' | '113 lbs' | '120 lbs' | '126 lbs' | '132 lbs' | '138 lbs' | 
  '144 lbs' | '150 lbs' | '157 lbs' | '165 lbs' | '175 lbs' | '190 lbs' | 
  '215 lbs' | '285 lbs';

export interface WrestlingThresholds {
  t1: number; // Power 4 D1 (Score: 95-99)
  t2: number; // Mid-Major D1 (Score: 85-94)
  t3: number; // Top D2 / D1 Walk-On (Score: 75-84)
  t4: number; // Solid D2 / High D3 (Score: 65-74)
  t5: number; // D3 / NAIA Prospect (Score: 55-64)
  t6: number; // Strong Varsity (Score: 40-54)
  t7: number; // Varsity Standard (Score: 20-39)
}

// Standards based on win-rate percentages and seasonal volume expectations
export const WRESTLING_METRIC_STANDARDS: Record<'Boys' | 'Girls', Record<string, WrestlingThresholds>> = {
  'Boys': {
    'WinPct': { t1: 90, t2: 80, t3: 70, t4: 60, t5: 50, t6: 40, t7: 30 },
    'PinsPerMatch': { t1: 0.7, t2: 0.6, t3: 0.5, t4: 0.4, t5: 0.3, t6: 0.2, t7: 0.1 },
    'TakedownsPerMatch': { t1: 4.0, t2: 3.2, t3: 2.5, t4: 1.8, t5: 1.2, t6: 0.8, t7: 0.4 },
    'CareerWins': { t1: 150, t2: 120, t3: 90, t4: 70, t5: 50, t6: 30, t7: 15 }
  },
  'Girls': {
    'WinPct': { t1: 85, t2: 75, t3: 65, t4: 55, t5: 45, t6: 35, t7: 25 },
    'PinsPerMatch': { t1: 0.8, t2: 0.65, t3: 0.5, t4: 0.35, t5: 0.25, t6: 0.15, t7: 0.05 },
    'TakedownsPerMatch': { t1: 3.5, t2: 2.8, t3: 2.0, t4: 1.4, t5: 1.0, t6: 0.6, t7: 0.3 },
    'CareerWins': { t1: 100, t2: 80, t3: 60, t4: 45, t5: 30, t6: 20, t7: 10 }
  }
};

export const LEVEL_MODIFIERS: Record<string, number> = {
  'JV / Dev Squad': 0.60,
  'Varsity Contributor': 0.85,
  'Varsity Starter': 1.0,
  'All-Conference Tier': 1.15,
  'All-State / National': 1.35,
  'Elite Club (ECNL / AAU / Next)': 1.50
};

export const calculateWrestlingMetricScore = (val: number, thresholds: WrestlingThresholds): number => {
  if (isNaN(val) || val < 0) return 10;
  if (val >= thresholds.t1) return Math.min(99, Math.round(95 + ((val - thresholds.t1) / (thresholds.t1 * 0.1)) * 4));
  if (val >= thresholds.t2) return Math.round(85 + ((val - thresholds.t2) / (thresholds.t1 - thresholds.t2)) * 10);
  if (val >= thresholds.t3) return Math.round(75 + ((val - thresholds.t3) / (thresholds.t2 - thresholds.t3)) * 10);
  if (val >= thresholds.t4) return Math.round(65 + ((val - thresholds.t4) / (thresholds.t3 - thresholds.t4)) * 10);
  if (val >= thresholds.t5) return Math.round(55 + ((val - thresholds.t5) / (thresholds.t4 - thresholds.t5)) * 10);
  if (val >= thresholds.t6) return Math.round(40 + ((val - thresholds.t6) / (thresholds.t5 - thresholds.t6)) * 14);
  if (val >= thresholds.t7) return Math.round(20 + ((val - thresholds.t7) / (thresholds.t6 - thresholds.t7)) * 19);
  return Math.max(10, Math.round((val / thresholds.t7) * 20));
};

export const compileWrestlingFitScore = (
  gender: string,
  levelOfPlay: string,
  matchesPlayed: number,
  rawMetrics: Record<string, number | null>
): { compositeScore: number; analyticalTrace: any[] } => {
  const targetedGender = (gender === 'Boys' || gender === 'Men') ? 'Boys' : 'Girls';
  const genderStandards = WRESTLING_METRIC_STANDARDS[targetedGender];
  
  const normalizedMatches = Math.max(1, matchesPlayed);
  const modifier = LEVEL_MODIFIERS[levelOfPlay] || 1.0;
  const trace: any[] = [];
  let accumulatedWeightedScore = 0;
  let totalWeightsAssigned = 0;

  const registerResult = (key: string, rawVal: number | null | undefined, thresholdKey: string, weight: number, isPerMatch = true) => {
    if (rawVal === null || rawVal === undefined || isNaN(rawVal)) return;

    const rate = isPerMatch ? (rawVal / normalizedMatches) : rawVal;
    const standards = genderStandards[thresholdKey];
    
    const modifiedValue = rate * modifier;
    const score = calculateWrestlingMetricScore(modifiedValue, standards);

    accumulatedWeightedScore += (score * weight);
    totalWeightsAssigned += weight;

    trace.push({
      metricLabel: key,
      perMatchRate: rate,
      calibratedScore: score,
      weightAllocation: `${weight * 100}%`
    });
  };

  // Logic: Win % is king, Takedowns measure aggressiveness, Pins measure finishing ability
  registerResult('Win %', rawMetrics.winPct, 'WinPct', 0.50, false);
  registerResult('Takedowns/Match', rawMetrics.takedowns, 'TakedownsPerMatch', 0.25);
  registerResult('Pins/Match', rawMetrics.pins, 'PinsPerMatch', 0.15);
  registerResult('Career Wins', rawMetrics.careerWins, 'CareerWins', 0.10, false);

  let finalComposite = totalWeightsAssigned > 0 ? Math.round(accumulatedWeightedScore / totalWeightsAssigned) : 0;
  
  // Ceiling caps for development levels
  if (levelOfPlay === 'JV / Dev Squad') finalComposite = Math.min(50, finalComposite);
  else if (levelOfPlay === 'Varsity Contributor') finalComposite = Math.min(70, finalComposite);

  return {
    compositeScore: Math.min(99, Math.max(10, finalComposite)),
    analyticalTrace: trace
  };
};