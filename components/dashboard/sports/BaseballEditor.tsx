...
import { compileBaseballFitScore, BaseballPositionGroup } from '@/utils/BaseballRecruitingEngine';

export interface BaseballEditorProps {
  baseballStats: any;
  genderKey: string;
...
export default function BaseballEditor({ baseballStats, genderKey, onSync, showToast }: BaseballEditorProps) {
  const currentPos = (baseballStats.position || 'Outfield (LF/CF/RF)') as BaseballPositionGroup;
  
  const [posGroup, setPosGroup] = useState<BaseballPositionGroup>(
    ['Pitcher (RHP/LHP)', 'Catcher (C)', 'First Base (1B)', 'Infield (2B/3B/SS)', 'Outfield (LF/CF/RF)'].includes(currentPos) 
      ? currentPos 
      : 'Outfield (LF/CF/RF)'
  );
  
  const [level, setLevel] = useState(baseballStats.level || 'Varsity Starter');
  const [games, setGames] = useState<number>(baseballStats.metaContext?.gamesPlayed || 25);
  const [isSaving, setIsSaving] = useState(false);

  const savedPayload = baseballStats.metaContext?.rawTotals || {};
  ...
  useEffect(() => {
    if (!baseballStats.metaContext?.rawTotals) {
      setBa(''); setOps(''); setHr(''); setRbi(''); setSb('');
      setVelocity(''); setEra('');
    }
  }, [posGroup, baseballStats.metaContext?.rawTotals]);

  const computedInputObject = useMemo(() => {
    const result: Record<string, number | null> = {};
    if (posGroup === 'Pitcher (RHP/LHP)') {
  ...
  const { compositeScore, analyticalTrace } = compileBaseballFitScore(
    genderKey,
    posGroup,
    level,
    games,
  ...
  const handleManualSave = async () => {
    ...
    const mockMetricsArray = analyticalTrace.map(t => ({
      name: t.metricLabel,
      value: t.perGameRate.toFixed(3) // Decimals look better for BA/OPS
    }));
    ...
    await onSync({
      ...
      metaContext: {
        gamesPlayed: games,
        rawTotals: posGroup === 'Pitcher (RHP/LHP)' 
          ? { velocity, era } 
          : { ba, ops, hr, rbi, sb }
      }
    });

    showToast("Baseball metrics verified and secured!", "success");
    setIsSaving(false);
  };

  const isPitcher = posGroup === 'Pitcher (RHP/LHP)';
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-slate-800/80 relative z-10">
        <div>
          <div className="flex items-center gap-2">
            <span className="p-2 bg-red-500/10 rounded-xl border border-red-500/20 text-red-500">
              <Target className="w-5 h-5" />
            </span>
            <h3 className="text-xl font-black tracking-tight">Baseball Contextual Normalizer</h3>
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">
            Engine evaluates <strong className="text-red-500">Averages & Volume Rates</strong> relative to games played.
          </p>
        </div>
        ...
        {compositeScore > 0 && (
          <div className="flex items-center gap-3 bg-slate-900/90 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-slate-800 shadow-inner w-full sm:w-auto justify-between sm:justify-start">
            <div className="text-left">
              ...
              <span className="text-xs font-black bg-gradient-to-r from-red-400 to-rose-400 bg-clip-text text-transparent">
                Dynamic Matrix Verified
              </span>
            </div>
            <div className="text-2xl font-black px-3 py-1 bg-gradient-to-br from-red-500 to-rose-600 text-white rounded-xl shadow-lg shrink-0 shadow-red-500/10">
              {compositeScore}
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-6 relative z-10">
        <div>
          ...
          <select value={posGroup} onChange={e => setPosGroup(e.target.value as any)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-slate-200 cursor-pointer">
            <option value="Pitcher (RHP/LHP)">Pitcher (RHP/LHP)</option>
            <option value="Catcher (C)">Catcher (C)</option>
            <option value="First Base (1B)">First Base (1B)</option>
            <option value="Infield (2B/3B/SS)">Infield (2B/3B/SS)</option>
            <option value="Outfield (LF/CF/RF)">Outfield (LF/CF/RF)</option>
          </select>
        </div>
        ...
        <div>
          ...
          <select value={level} onChange={e => setLevel(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-slate-200 cursor-pointer">
            ...
            <option value="Elite Club (Perfect Game / PBR)">Elite Club (Perfect Game / PBR)</option>
          </select>
        </div>
        ...
        {isPitcher ? (
          <>
            <div className="md:col-span-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
                ...
                <div className="relative group inline-block">
                  <HelpCircle className="w-3.5 h-3.5 text-slate-500 hover:text-red-500 cursor-help transition-colors" />
              ...
              <input type="text" inputMode="decimal" placeholder="(e.g. 88)" value={velocity} onChange={e => setVelocity(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-white placeholder-slate-700 shadow-inner focus:border-red-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" inputMode="decimal" placeholder="(e.g. 2.45)" value={era} onChange={e => setEra(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-white placeholder-slate-700 shadow-inner focus:border-red-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
            </div>
          </>
        ) : (
          <>
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
                ...
                <div className="relative group inline-block">
                  <HelpCircle className="w-3.5 h-3.5 text-slate-500 hover:text-red-500 cursor-help transition-colors" />
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-slate-800 text-slate-200 text-xs p-2.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-xl border border-slate-700 normal-case tracking-normal">
                    Enter as decimal (e.g. 0.345)
                    <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-800"></div>
                  </div>
                </div>
              </label>
              <input type="text" inputMode="decimal" placeholder="(e.g. 0.350)" value={ba} onChange={e => setBa(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-white placeholder-slate-700 shadow-inner focus:border-red-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
            </div>
            ...
            <div>
              ...
              <input type="text" inputMode="decimal" placeholder="(e.g. 1.050)" value={ops} onChange={e => setOps(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-white placeholder-slate-700 shadow-inner focus:border-red-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
            </div>
            ...
            <div>
              ...
              <input type="text" inputMode="decimal" placeholder="(Optional)" value={hr} onChange={e => setHr(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-white placeholder-slate-700 shadow-inner focus:border-red-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
            </div>
            ...
            <div>
              ...
              <input type="text" inputMode="decimal" placeholder="(Optional)" value={rbi} onChange={e => setRbi(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-white placeholder-slate-700 shadow-inner focus:border-red-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
            </div>
            ...
            <div>
              ...
              <input type="text" inputMode="decimal" placeholder="(Optional)" value={sb} onChange={e => setSb(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold outline-none text-white placeholder-slate-700 shadow-inner focus:border-red-500/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
            </div>
          </>
        )}
      </div>
      ...
      <div className="pt-6 relative z-10 flex flex-col md:flex-row gap-4 items-center justify-between border-t border-slate-800/80 mt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {analyticalTrace.map((block, idx) => (
                  <div key={idx} className="bg-slate-950/90 border border-slate-900 p-2.5 rounded-xl flex flex-col text-[11px] flex-1 min-w-[120px]">
                    ...
                    <span className="text-[10px] font-bold text-red-500 mt-0.5">Yield Index: {block.calibratedScore}/99</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="w-full md:w-auto self-stretch flex items-end">
          <button 
            ...
            disabled={isSaving}
            className="w-full md:w-auto h-14 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-black px-8 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
          >
